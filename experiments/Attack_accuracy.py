    ...
    for dataset in list_datasets:
        ...
        for sgd_it in list_local_sgd_it:
            ...
            for run in range(1,runs+1):
                ...
                accuracy = np.load(prefix + f"{pre}_accuracy_val_{extra}{args.dataset}_{args.epochs}_{args.num_users}_{args.frac}_{args.local_bs * args.local_sgd_it}_{args.local_bs}_{args.local_sgd_it}_{args.attacker_model_reg}_{args.penality}_{args.prop}_weights_{run}.npy")

                
                list_stack_accuracy.append(accuracy)
                
            print("list_stack_accuracy[0].shape[0]:",list_stack_accuracy[0].shape[0])
            print("shape:",np.array(list_stack_accuracy).shape)

            accuracy=np.mean(np.array(list_stack_accuracy).reshape(3,list_stack_accuracy[0].shape[0]),axis=0)

            START_ROUND = 10
            INTERVAL_ROUND = 10 # perform reconstruction in every INTERVAL_ROUND

            round_num = len(accuracy)
            ROUNDS = np.arange(START_ROUND, round_num, INTERVAL_ROUND)

            marker = itertools.cycle(('o', '+', 'x', '*', '.', 'X'))
            
            plt.plot(ROUNDS, accuracy[ROUNDS], marker = next(marker))

            #plt.plot(ROUNDS, accuracy[ROUNDS], label="Accuracy",  marker = next(marker))
            ...
            plt.savefig(f'./Results_CCS/Attack_accuracy_figures/Attack-accuracy-{args.dataset}_{args.epochs}_{args.num_users}_{args.frac}_{args.local_bs * args.local_sgd_it}_{args.mia_sample_num}_{attack}.png')
