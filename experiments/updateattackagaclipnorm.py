...
class LocalUpdate(object):
    def __init__(self, args, dataset, idxs, logger, fixed_train_sample=None, train_ratio=0.8, flag=0):
        self.args = args
        self.logger = logger
    ...
    def update_weights(self, model, global_round):
        ...
        if(self.flag==1):
            lr=copy.deepcopy(self.args.lr) #/8)
        else:
            lr=copy.deepcopy(self.args.lr)
        ...
        #if(self.flag==1):
        #    model.load_state_dict(get_inverse_gradient(ref_model,model))
        #    return model, sum(epoch_loss) / len(epoch_loss)
        #else:
        #    return model, sum(epoch_loss) / len(epoch_loss)
...
def get_inverse_gradient(global_model,updated_model):
    w_g=copy.deepcopy(global_model.state_dict())
    w_u=copy.deepcopy(updated_model.state_dict())
    
    for key in list(w_g.keys()):
        extracted_gradients=torch.multiply(copy.deepcopy(w_u).get(key)-copy.deepcopy(w_g).get(key),1/40.0) # modifie ici
        w_u[key]=copy.deepcopy(w_g).get(key)+extracted_gradients
