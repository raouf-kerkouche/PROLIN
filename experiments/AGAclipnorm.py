...
class AGA(object):
    ...
    def _train_target_model(self, target_idx, local_sgd_it, sample_num=200):
        ...
        def train_step_honest(images, labels):
            ...
            # Gradient Norm Clipping to avoid exploding gradients
            nn.utils.clip_grad_norm_(target_model.parameters(), max_norm=2.0, norm_type=2)

            #Gradient Value Clipping
            #nn.utils.clip_grad_value_(model.parameters(), clip_value=1.0)
            optimizer.step()
            
        def train_step_malicious(images, labels):
            target_model.zero_grad()
            log_probs = target_model(images)
            loss = -criterion(log_probs, labels)
            loss.backward()
        ...
        for _ in tsamples:
            tsamples.set_description(f"Generating training data for AGA")
            ...
            for _ in range(self.args.local_ep):
                ...
                for images, labels in loader:
                    ...
                    train_step_malicious(images, labels)

            aga_dataset.append((self.get_features(target_model, init_model), True))
            ...
            aga_dataset.append((self.get_features(target_model, init_model), False))
    ...
    def get_features(self, local_model, global_model, corr_factor = 1.0):
        ...
        #updates= flatten_params(local_model) - torch.mul(corr_factor, flatten_params(global_model))
        #return torch.where(torch.isnan(updates), torch.zeros_like(updates), updates)

    def train(self, model, target_idx, batch_size = 16): #16
        # Generate training data for AGA
        aga_dataset = self._train_target_model(target_idx, self.args.local_sgd_it, sample_num=len(self.idxs))#self.args.mia_sample_num)
        
        print("HNA")

        self.train_data, self.test_data = random_split(aga_dataset, [int(0.8 * len(aga_dataset)) , len(aga_dataset) - int(0.8 * len(aga_dataset))])
        ...
        for epoch in range(self.args.attacker_model_ep):
            ...
            with tqdm(self.trainloader, unit="batch") as tepoch:
                ...
                for updates, labels in tepoch:
                    ...
                    #outputs = torch.where(torch.isneginf(outputs), torch.zeros_like(outputs), outputs)
                    #outputs = torch.where(torch.isposinf(outputs), torch.ones_like(outputs), outputs)
