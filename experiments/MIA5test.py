...
class MIA(object):
    def __init__(self, args, dataset=None, target_model=None, idxs=None, logger=None, lr=0.01, scaler = None):
        self.args = args
        self.logger = logger
        self.target_dataset = dataset

        # Default criterion set to BCE for logistic regression
        self.mia_criterion = nn.BCELoss().to(self.args.device)
        self.lr = lr
        self.idxs = idxs
    ...
    def _train_target_model(self, target_idx, local_sgd_it, sample_num=200):
        ...
        for _ in tsamples:
            tsamples.set_description(f"Generating training data for MIA")

            target_model.load_state_dict(init_model.state_dict())
            target_model.train()   

            # Sample IN (with target sample -> label = True)
            _train_data = self._get_train_data(self.args.local_bs * local_sgd_it, target_idx=target_idx)
            ...
            mia_dataset.append((self.get_features(target_model, init_model), True))
            ...
            mia_dataset.append((self.get_features(target_model, init_model), False))
    ...
    def train(self, model, target_idx, batch_size = 16): #16
        # Generate training data for MIA
        mia_dataset = self._train_target_model(target_idx, self.args.local_sgd_it, sample_num=len(self.idxs))#self.args.mia_sample_num)
        
        print("HNA")

        self.train_data, self.test_data = random_split(mia_dataset, [int(0.8 * len(mia_dataset)) , len(mia_dataset) - int(0.8 * len(mia_dataset))])
        ...
        for epoch in range(self.args.attacker_model_ep):
            ...
            with tqdm(self.trainloader, unit="batch") as tepoch:
                ...
                for updates, labels in tepoch:
                    ...
                    loss = self.mia_criterion(outputs, labels.to(torch.float32)) + add_reg
        ...
        print("Best_MIA_epoch:",ep_tmp)
        print("Best_MIA_acc:",acc_max)
    ...
    def predict(self, model, updates, labels, scale=False):
        ...
        return torch.sum(torch.eq(predictions.to(self.args.device), labels.to(self.args.device))).item(), predictions.to(self.args.device), outputs # correct predictions
